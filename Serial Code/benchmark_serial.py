"""
Script để đo hiệu suất Serial Adaptive Huffman Coding
Chạy tất cả test files và lưu kết quả vào CSV
"""
...
def compile_serial():
    ...
    try:
        result = subprocess.run([
            "g++", "-fopenmp", "-o", "serial.exe", "serial.cpp"
...
def run_serial_test(test_file):
    """Chạy test với file cụ thể và extract thời gian encoding, decoding và compression ratio"""
    print(f"Running test: {test_file}")
    
    try:
        # Chạy serial.exe với test file (test files ở ../Tests/)
        result = subprocess.run([
            "./serial.exe", test_file
        ], capture_output=True, text=True, check=True, timeout=300)  # 5 minutes timeout
...
def main():
    ...
    # Compile serial code
    if not compile_serial():
        return
    
    # Test files (bỏ qua repeat.txt và test.txt)
    ...
    for test_file in test_files:
        ...
        for run in range(5):
            print(f"  Run {run + 1}/5...")
            test_result = run_serial_test(test_file)
        ...
        if encoder_times:
            ...
            print(f"All runs failed for {test_file}.txt")
    ...
    if results:
        ...
        csv_file = os.path.join(results_dir, "serial_benchmark_results.csv")
        ...
        with open(csv_file, 'w', newline='', encoding='utf-8') as f:
            ...
            # Header
            writer.writerow(['File', 'Encoder_Time_seconds', 'Decoder_Time_seconds', 'Compression_Ratio', 'Runs'])
        ...
        # Display summary
        print(f"\nSummary")
        print(f"{'File':<15} {'Encoder(s)':<12} {'Decoder(s)':<12} {'Comp.Ratio':<12} {'Status':<10}")
        print("-" * 70)
        
        for result in results:
            print(f"{result['file']:<15} {result['avg_encoder_time']:<12.6f} {result['avg_decoder_time']:<12.6f} {result['avg_compression_ratio']:<12.6f} {'Success':<10}")
